...
// Chapman
//***
// Due to errors in given source code file, project was not able to be complete.
//***
...
bool Safe(){
    ...
    for(i=0;i<m;i++){
        if(Finish[i]==true){
            ...
            for(int j = 0; j < n; j++) {
                if(Available[j] < Need[i][j]) {
                    // If the available resources of type j are less than the need of process i for resource j,
                    // then the resources do not meet the need.
                    cout << "Resources do not meet the need for process " << i << " for resource type " << j << endl;
                    break;
                }
            }
            ...
            bool allResourcesMeetNeed = true;
            for(int j = 0; j < n; j++) {
                if(Need[i][j] > Work[j]) {
                    allResourcesMeetNeed = false;
                    break;
                }
            }

            if(allResourcesMeetNeed) {
                // All types of resources meet the need for process i
                // Resources can be truly assigned to this process
            }
            ...
            { 
                ...
                // Assuming p is an array or vector and l is an integer index
                p[l] = i; // Record the number of process i into safe sequence p[l]
                l++; // Increment l to point to the next position of p
        ...
        if(l == m) {
            // We have finished recording all process numbers in the safe sequence
        }
...
void Bank() // banker's algorithm
{
    ...
    while(1)
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            Allocation[cusneed][i] += Need[cusneed][i];
        ...
        if(Safe()) {
            // The system is safe
            // You can put the code to execute when the system is safe here
        } else {
            // The system is not safe
            // You can put the code to execute when the system is not safe here
        }
